# Remote Features Support Design
...
Once implemented, create `examples/with-remote-features/`:
...
### Image-based configs with features (crib bug)

Currently, using features with `"image"` in devcontainer.json generates invalid Dockerfiles with duplicate `FROM` statements. This affects both local and remote features.

**Workaround:** Use Dockerfile-based configs (`"build": {"dockerfile": "..."}`) with features instead.

**Root cause:** `engine/build.go` line 59 in `buildFromImage()` concatenates feature prefix + actual FROM + feature content, causing two FROM statements.

**Fix needed:** When building from an image with features:
1. Use feature prefix (syntax + placeholder ARG)
2. Replace placeholder ARG with actual image name OR embed real image in feature generation
3. Omit the second FROM from feature content

## Future Enhancements

- OCI Registry mirror support (for air-gapped environments)
- Feature collection support (e.g., `ghcr.io/devcontainers/features` as a collection URL)
- Registry credential management via `crib config` command
- Offline mode: pre-cache features for a project
- Fix image+features path to work correctly (see Known Issues)
